...
fieldValueSortTest = testGroup "field value"
  ...
  [ testCase "int32" $ 42 <? 2112 $ fieldValueSort Int32Field
  , testCase "string" $ "a" <? "aa" $ fieldValueSort StringField
  -- Enum values.
  , testCase "enum Bar pos" $ BAR3 <? BAR5 $ fieldValueSort EnumField
  , testCase "enum Bar neg" $ NEGATIVE <? BAR5 $ fieldValueSort EnumField
  , testCase "enum Bar eq" $
      comparesAs EQ BAR5 BAR5 $ fieldValueSort EnumField
  -- Message fields.
  , testCase "message Foo eq" $
      comparesAs EQ def def $
      fieldValueSort (MessageField :: FieldTypeDescriptor Foo)
...
protoMapSortTest = testGroup "map"
  ...
  where
    c = protoMapAssocs sorting1 sorting _1 _2
...
fieldSortTest = testProperty "compares by field" $
    \(ArbitraryMessage msg1 :: ArbitraryMessage Foo) (ArbitraryMessage msg2) ->
      compare (fromEnum <$> msg1 ^. maybe'bar) (fromEnum <$> msg2 ^. maybe'bar)
        == sortCompare (fieldSort fieldDesc) msg1 msg2
  where
    fieldDesc =
